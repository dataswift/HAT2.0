...
trait JwtTokenHandler {
  ...
  def fetchOrGenerateToken(user: User): Future[AccessToken] = {
    ...
    maybeToken flatMap {
      ...
      case None =>
        val newAccessToken = UserAccessTokenRow(getJwtToken(user.userId), user.userId)
  ...
  def getJwtToken(userId: UUID): String = {
    val issuedAt = new DateTime()
    val expiresAt = issuedAt.plusMinutes(5)
  ...
  def validateJwtToken(token: String): Boolean = {
    ...
    val fresh = claimSet.getExpirationTime.before(DateTime.now().toDate)
