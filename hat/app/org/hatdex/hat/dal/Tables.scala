...
trait Tables {
   ...
   */
  case class SheFunctionRow(name: String, description: String, trigger: play.api.libs.json.JsValue, enabled: Boolean, bundleId: String, lastExecution: Option[org.joda.time.DateTime] = None, headline: String = "", dataPreview: Option[play.api.libs.json.JsValue] = None, logo: Option[String] = None)
  ...
  implicit def GetResultSheFunctionRow(implicit e0: GR[String], e1: GR[play.api.libs.json.JsValue], e2: GR[Boolean], e3: GR[Option[org.joda.time.DateTime]], e4: GR[Option[play.api.libs.json.JsValue]], e5: GR[Option[String]]): GR[SheFunctionRow] = GR {
    prs =>
      import prs._
      SheFunctionRow.tupled((<<[String], <<[String], <<[play.api.libs.json.JsValue], <<[Boolean], <<[String], <<?[org.joda.time.DateTime], <<[String], <<?[play.api.libs.json.JsValue], <<?[String]))
  }
  ...
  class SheFunction(_tableTag: Tag) extends profile.api.Table[SheFunctionRow](_tableTag, Some("hat"), "she_function") {
    def * = (name, description, trigger, enabled, bundleId, lastExecution, headline, dataPreview, logo) <> (SheFunctionRow.tupled, SheFunctionRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(name), Rep.Some(description), Rep.Some(trigger), Rep.Some(enabled), Rep.Some(bundleId), lastExecution, Rep.Some(headline), dataPreview, logo).shaped.<>({ r => import r._; _1.map(_ => SheFunctionRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6, _7.get, _8, _9))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
