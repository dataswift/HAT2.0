...
class InstagramMediaMapper extends DataEndpointMapper {
  ...
  private def instagramApiv2(content: JsValue): Try[DataFeedItem] = {
    for {
      createdTime <- Try(new DateTime((content \ "ds_created_time").as[String].toLong * 1000))
...
class InstagramProfileStaticDataMapper extends StaticDataEndpointMapper {
  def dataQueries(): Seq[PropertyQuery] = {
    val propertyQueryv1 = PropertyQuery(
      ...
      Some("hat_created_time"), Some("descending"), Some(1))
    val propertyQueryv2 = PropertyQuery(
      List(EndpointQuery("instagram/profile", None, None, None)),
      Some("ds_created_time"), Some("descending"), Some(1))
  ...
  private def instagramApiv1(lastPartOfEndpointString: String, value: JsObject): Seq[StaticDataValues] = {
    ...
    maybeTransformedData match {
      ...
      case e: JsError =>
        logger.error(s"Couldn't validate static data JSON for $lastPartOfEndpointString. $e")
        Seq()
    }
  }
  ...
  private def instagramApiv2(lastPartOfEndpointString: String, value: JsObject): Seq[StaticDataValues] = {
