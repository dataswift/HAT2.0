    ...
    hatServerProvider.retrieve(hat) flatMap {
      case Some(hatServer) ⇒
        functionService.all(active = true)(hatServer.db)
          .map(
            _.filter({
              case FunctionConfiguration(_, _, FunctionTrigger.TriggerPeriodic(period), true, true, _, Some(lastExecution)) if lastExecution.isBefore(DateTime.now().minus(period)) ⇒ true
              case FunctionConfiguration(_, _, FunctionTrigger.TriggerPeriodic(_), true, true, _, None) ⇒ true // no execution recoded yet
              case FunctionConfiguration(_, _, FunctionTrigger.TriggerIndividual(), true, true, _, _) ⇒ true
              case _ ⇒ false
            })
