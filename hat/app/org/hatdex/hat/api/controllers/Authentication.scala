...
class Authentication @Inject() (
  ...
  val onlyHatOwnerCanReset = Unauthorized(
    Json.toJson(
      ErrorMessage(
        "Password reset unauthorized",
        "Only HAT owner can reset their password"
      )
    )
  )
  ...
  def handleClaimStart(lang: Option[String]): Action[ApiClaimHatRequest] =
    UserAwareAction.async(parsers.json[ApiClaimHatRequest]) { implicit request =>
  ...
  def handleClaimComplete(claimToken: String): Action[HatClaimCompleteRequest] =
    UserAwareAction.async(parsers.json[HatClaimCompleteRequest]) { implicit request =>
  ...
  private def updateHatMembership(
      claim: HatClaimCompleteRequest): Future[Done] = {
  ...
  def handleRevalidation: Action[ApiValidationRequest] =
    UserAwareAction.async(parsers.json[ApiValidationRequest]) { implicit request =>
  ...
  def sendRevalidationEmail(
      email: String,
      applicationId: String,
      requestHost: String,
      lang: Lang
    )(implicit hatServer: HatServer): Unit = {
    implicit val l = lang
    val token      = MailTokenUser(email, isSignUp = false)
    // Store that token
