...
class ContractFilesImpl @Inject() (
  ...
  override def startUpload: Action[ContractFile] =
  ...
  override def completeUpload(fileId: String): Action[ContractDataReadRequest] =
  ...
  override def getDetail(fileId: String): Action[ContractDataReadRequest] =
  ...
  override def getContent(fileId: String): Action[ContractDataReadRequest] =
    contractAction.doWithContract(parsers.json[ContractDataReadRequest], None, isWriteAction = false) {
      (_, user, hatServer, maybeAuthenticator) =>
        fileUploadService
  ...
  override def updateFile(fileId: String): Action[ContractFile] =
    contractAction.doWithContract(parsers.json[ContractFile], None, isWriteAction = true) {
  ...
  override def deleteFile(fileId: String): Action[ContractDataReadRequest] =
    contractAction.doWithContract(parsers.json[ContractDataReadRequest], None, isWriteAction = true) {
      (_, user, hatServer, maybeAuthenticator) =>
        val result =
          for {
