...
class Applications @Inject() (
    ...
    applicationsService: ApplicationsService) extends HatApiController(components, silhouette)
                  ...
                  InternalServerError(
                    Json.toJson(
                      ErrorMessage(
                        ...
                        s"Application $id bundle ${
                          status.application.permissions.dataRequired
                            .map(_.bundle.name)
                        } clashes with one already setup")))
              }
          } getOrElse {
            Future.successful(
              BadRequest(
  ...
  def applicationToken(id: String): Action[AnyContent] =
    SecuredAction(
      ContainsApplicationRole(RetrieveApplicationToken(id)) || WithRole(Owner())).async { implicit request =>
        applicationsService
