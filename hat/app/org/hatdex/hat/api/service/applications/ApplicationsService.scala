...
class ApplicationsService @Inject() (
  ...
  private def setupApplication(
      ...
      user: HatUser,
      requestHeader: RequestHeader
    ): Future[HatApplication] = {
    for {
      _ <- joinContract(application.application, hat.hatName)
      _ <-
        applicationSetupStatusUpdate(application, enabled = true)(
          hat.db
        ) // Update status
    ...
    val maybeDependenciesSetup: Option[Future[Boolean]] =
      application.application.dependencies.map { deps =>
        ...
        val dependenciesEnabled = for {
          ...
          toolStatuses <-
            Future.sequence(toolDeps.map(fId => functionService.get(fId)))
        ...
        } yield updatedAppStatuses.forall(app =>
          app.setup && app.enabled
        ) && updatedToolStatuses.forall(_.status.enabled)
  ...
  def disable(
    ...
    for {
      ...
      _ <-
        applicationSetupStatusUpdate(application, enabled = false)(
          hat.db
        ) // Update status
      app <-
        applicationStatus(
  ...
  protected def applicationSetupStatus(
    ...
    ): Future[Seq[ApplicationStatusRow]] = {
  ...
  protected def applicationSetupStatus(
    ...
    ): Future[Option[ApplicationStatusRow]] = {
