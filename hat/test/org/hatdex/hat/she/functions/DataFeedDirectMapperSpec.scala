...
class DataFeedDirectMapperSpec extends PlaySpecification with Mockito with DataFeedDirectMapperContext with BeforeAfterAll {
  ...
  "The `InstagramMediaMapper` class" should {
    ...
    "create data queries using correct unix timestamp format" in {
      ...
      propertyQuery.head.endpoints.head.filters.get.head.operator.asInstanceOf[Between].lower.as[String] must be equalTo "2018-05-01T10:00:00.000+01:00"
      propertyQuery.head.endpoints.head.filters.get.head.operator.asInstanceOf[Between].upper.as[String] must be equalTo "2018-05-02T10:00:00.000+01:00"
    }
  }
