...
trait FunctionServiceContext extends HATTestContext {
  ...
  val dummyFunctionConfiguration: FunctionConfiguration = FunctionConfiguration(
    ...
    FunctionInfo(
      ...
      "contact@email.com",
      None,
      ApplicationGraphics(Drawable(None, "", None, None),
  ...
  val unavailableFunctionConfiguration: FunctionConfiguration = dummyFunctionConfiguration.copy(
    ...
    info =
      dummyFunctionConfiguration.info.copy(name = "test-test-unavailable-function", headline = "Unavailable Function"),
  ...
  class registeredFunction extends FunctionExecutable {
    val configuration: FunctionConfiguration = dummyFunctionConfiguration.copy(
      ...
      dataBundle = EndpointDataBundle(
        "data-feed-counter",
        Map(
          "facebook/feed" -> PropertyQuery(List(EndpointQuery("facebook/feed", None, None, None)),
                                           Some("created_time"),
                                           Some("descending"),
                                           None
              ),
          "facebook/events" -> PropertyQuery(List(EndpointQuery("facebook/events", None, None, None)),
                                             Some("start_time"),
                                             Some("descending"),
                                             None
              ),
          "twitter/tweets" -> PropertyQuery(List(EndpointQuery("twitter/tweets", None, None, None)),
                                            Some("lastUpdated"),
                                            Some("descending"),
                                            None
              ),
          "fitbit/sleep" -> PropertyQuery(List(EndpointQuery("fitbit/sleep", None, None, None)),
                                          Some("endTime"),
                                          Some("descending"),
                                          None
              ),
          "fitbit/activity" -> PropertyQuery(List(EndpointQuery("fitbit/activity", None, None, None)),
                                             Some("originalStartTime"),
                                             Some("descending"),
                                             None
              ),
          "fitbit/weight" -> PropertyQuery(List(EndpointQuery("fitbit/weight", None, None, None)),
                                           Some("date"),
                                           Some("descending"),
                                           None
              ),
          "calendar/google/events" -> PropertyQuery(List(EndpointQuery("calendar/google/events", None, None, None)),
                                                    Some("start.dateTime"),
                                                    Some("descending"),
                                                    None
              ),
          "notables/feed" -> PropertyQuery(List(EndpointQuery("rumpel/notablesv1", None, None, None)),
                                           Some("created_time"),
                                           Some("descending"),
                                           None
              ),
          "spotify/feed" -> PropertyQuery(List(EndpointQuery("spotify/feed", None, None, None)),
                                          Some("played_at"),
                                          Some("descending"),
                                          None
              ),
          "monzo/transactions" -> PropertyQuery(List(EndpointQuery("monzo/transactions", None, None, None)),
                                                Some("created"),
                                                Some("descending"),
                                                None
              )
    ...
    def execute(
      ...
      Future.successful(
        request.data.values.flatten
          .map(d => Response(namespace, endpoint, Seq(d.data.as[JsObject].+("mapped" -> JsBoolean(true))), Seq()))
          .toSeq
      )
  ...
  val registeredDummyFunctionAvailable: FunctionExecutable = new DummyFunctionExecutable(
    dummyFunctionConfigurationAvailable
  )
}
