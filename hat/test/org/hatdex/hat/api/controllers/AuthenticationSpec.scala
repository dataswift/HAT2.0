...
import org.hatdex.hat.phata.models.{ApiPasswordChange, ApiPasswordResetRequest, ApiPasswordValidationRequest, MailTokenUser}
...
//    }
    ...
    "The `handleResendValidation` method should" in {
      "RESEND: Hide the fact that email doesn't match by returning status 200" in {
        val request = FakeRequest("POST", "http://hat.hubofallthings.net")
          .withAuthenticator(dataDebitUser.loginInfo)
      ...
      "RESEND: Send email to the owner if provided email matches" in {
        val request = FakeRequest("POST", "http://hat.hubofallthings.net")
          .withAuthenticator(dataDebitUser.loginInfo)
          .withJsonBody(Json.toJson(passwordValidationIncorrect))
        ...
        println(ret)
        status(result) must equalTo(OK)
//        there was one(mockMailer).claimHat(
//          any[String],
//          any[String],
//          any[Option[org.hatdex.hat.utils.ApplicationMailDetails]]
//        )(any[MessagesApi],
//          any[Lang],
//          any[HatServer])
      }
...
trait AuthenticationContext extends HATTestContext {
  ...
  val passwordValidationIncorrect = ApiPasswordValidationRequest("email@example.com")
  val passwordValidationOwner = ApiPasswordValidationRequest("user@hat.org")
}
