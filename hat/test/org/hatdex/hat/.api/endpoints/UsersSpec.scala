...
import org.hatdex.hat.api.{TestDataCleanup, DatabaseInfo}
import org.hatdex.hat.api.endpoints.jsonExamples.UserExamples
...
class UsersSpec extends Specification with Specs2RouteTest with BeforeAfterAll with Users {
  ...
  def beforeAll() = {
    ...
    validAccessTokens = Future.sequence {
      validUsers.map { user => fetchOrGenerateToken(HatUser.fromDbModel(user), "hat.hubofallthings.net", user.role) }
    }
